...
void log_new_percentage(int new_bat_value, int is_charging)
{
	...
	if (bat_log_db != NULL) {
		char *sql = sqlite3_mprintf(
			"INSERT INTO bat_activity(device_serial, bat_level, duration, is_charging) VALUES(%Q, %d, %d, %d);",
			device_model, new_bat_value, 0, is_charging);
...
int get_current_session_time(void)
{
	...
	if (bat_log_db != NULL) {
		const char *sql =
			"SELECT * FROM bat_activity WHERE device_serial = ? AND is_charging = 1 ORDER BY id DESC LIMIT 1;";
		sqlite3_stmt *stmt;
		int rc = sqlite3_prepare_v2(bat_log_db, sql, -1, &stmt, 0);
...
int main(int argc, char *argv[])
{
	...
	while (!quit) {
		...
		if (pwr.is_charging) {
			...
			LOG_debug("Charging stopped: suspended = %d, perc = %d\n", is_suspended, pwr.charge);
		...
		if (!is_suspended) {
			if (ticks >= CHECK_BATTERY_TIMEOUT_S) {
				LOG_debug("battery check: suspended = %d, perc = %d\n", is_suspended, pwr.charge);
			...
			if (pwr.charge != old_percentage) {
				...
				LOG_debug("saving percBat: suspended = %d, perc = %d\n", is_suspended, pwr.charge);

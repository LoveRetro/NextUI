...
#define CHAR_COLON 11
	while (c = chars[i]) {
		...
		// SCALE1(DIGIT_WIDTH)), y, SCALE1(DIGIT_HEIGHT), SCALE1(DIGIT_HEIGHT) - digit->h); fflush(stdout);
	...
	while (!quit) {
		...
		if (dirty) {
			...
			blitBar(x, y,
					(select_cursor == CURSOR_YEAR ? SCALE1(40) : (select_cursor == CURSOR_AMPM ? ampm_w : SCALE1(20))));

...
inline void GFX_blitTextCPP(TTF_Font *font, const char *str, int leading, SDL_Color color, SDL_Surface *dst,
...
class AbstractMenuItem
{
  ...
  public:
	AbstractMenuItem(ListItemType type, const std::string &name, const std::string &desc,
					 ...
					 MenuList *submenu = nullptr)
		: type(type), name(name), desc(desc), on_get(on_get), on_set(on_set), on_reset(on_reset),
		  on_confirm(on_confirm), submenu(submenu)
	{
	}
	~AbstractMenuItem()
	{
	...
	virtual InputReactionHint handleInput(int &dirty)
	{
		return Unhandled;
	};
	...
	const ListItemType getType() const
	{
		return type;
	}

	virtual void drawCustomItem(SDL_Surface *surface, const SDL_Rect &dst, const AbstractMenuItem &item,
								bool selected) const
	{
	}

	virtual const std::vector<std::any> getValues() const
	{
		return {getValue()};
	};
...
{
  public:
	StaticMenuItem(ListItemType type, const std::string &name, const std::string &desc, ValueGetCallback on_get)
...
class MenuItem : public AbstractMenuItem
{
  ...
  public:
	MenuItem(ListItemType type, const std::string &name, const std::string &desc, const std::vector<std::any> &values,
			 const std::vector<std::string> &labels, ValueGetCallback on_get = nullptr,
			 ValueSetCallback on_set = nullptr, ValueResetCallback on_reset = nullptr,
			 MenuListCallback on_confirm = nullptr, MenuList *submenu = nullptr);

	MenuItem(ListItemType type, const std::string &name, const std::string &desc, const std::vector<std::any> &values,
			 ValueGetCallback on_get = nullptr, ValueSetCallback on_set = nullptr,
			 ValueResetCallback on_reset = nullptr, MenuListCallback on_confirm = nullptr, MenuList *submenu = nullptr);

	MenuItem(ListItemType type, const std::string &name, const std::string &desc, int min, int max,
			 const std::string suffix, ValueGetCallback on_get = nullptr, ValueSetCallback on_set = nullptr,
			 ValueResetCallback on_reset = nullptr, MenuListCallback on_confirm = nullptr, MenuList *submenu = nullptr);

	MenuItem(ListItemType type, const std::string &name, const std::string &desc, MenuListCallback on_confirm = nullptr,
			 MenuList *submenu = nullptr);
	...
	const std::vector<std::any> getValues() const override
	{
		return values;
	}
...
class MenuList
{
  ...
  public:
	MenuList(MenuItemType type, const std::string &desc, std::vector<AbstractMenuItem *> items,
			 MenuListCallback on_change = nullptr, MenuListCallback on_confirm = nullptr);
	~MenuList();

	...
	: MenuList(MenuItemType::Fixed, "Network", {}), globalQuit(globalQuit), globalDirty(globalDirty)
{
	...
	diagItem = new MenuItem(ListItemType::Generic, "WiFi diagnostics", "Enable/disable WiFi logging", {false, true},
							{"Off", "On"}, std::bind(&Menu::getWifDiagnosticsState, this),
			...
			if (!menuOpen) {
...

				{
					...
					for (auto &[s, r] : scanSsids) {
						...
						MenuList *options;
						if (connected)
							options = new MenuList(
								MenuItemType::List, "Options",
							...
							options = new MenuList(
								MenuItemType::List, "Options",
								{new ConnectKnownItem(r, selectionDirty), new ForgetItem(r, selectionDirty)});
						else
							options =
								new MenuList(MenuItemType::List, "Options", {new ConnectNewItem(r, selectionDirty)});
...
ConnectKnownItem::ConnectKnownItem(WIFI_network n, bool &dirty)
	: MenuItem(ListItemType::Button, "Connect", "Connect to this network.",
			   [&](AbstractMenuItem &item) -> InputReactionHint {
...
ConnectNewItem::ConnectNewItem(WIFI_network n, bool &dirty)
	: MenuItem(ListItemType::Button, "Enter WiFi passcode", "Connect to this network.", DeferToSubmenu,
			   new KeyboardPrompt("Enter Wifi passcode",
								  [&](AbstractMenuItem &item) -> InputReactionHint {
...
void NetworkItem::drawCustomItem(SDL_Surface *surface, const SDL_Rect &dst, const AbstractMenuItem &item,
								 bool selected) const
{
	...
	// wifi icon
	auto asset = net.rssi >= -60 ? ASSET_WIFI :		   // anything above 61

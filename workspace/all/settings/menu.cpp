...
MenuItem::MenuItem(ListItemType type, const std::string &name, const std::string &desc,
				   const std::vector<std::any> &values, const std::vector<std::string> &labels, ValueGetCallback on_get,
				   ValueSetCallback on_set, ValueResetCallback on_reset, MenuListCallback on_confirm, MenuList *submenu)
	: AbstractMenuItem(type, name, desc, on_get, on_set, on_reset, on_confirm, submenu), values(values), labels(labels)
{
	initSelection();
}

MenuItem::MenuItem(ListItemType type, const std::string &name, const std::string &desc,
				   const std::vector<std::any> &values, ValueGetCallback on_get, ValueSetCallback on_set,
				   ValueResetCallback on_reset, MenuListCallback on_confirm, MenuList *submenu)
	: AbstractMenuItem(type, name, desc, on_get, on_set, on_reset, on_confirm, submenu), values(values) /*labels({}),*/
{
...
MenuItem::MenuItem(ListItemType type, const std::string &name, const std::string &desc, int min, int max,
				   const std::string suffix, ValueGetCallback on_get, ValueSetCallback on_set,
				   ValueResetCallback on_reset, MenuListCallback on_confirm, MenuList *submenu)
...
MenuItem::MenuItem(ListItemType type, const std::string &name, const std::string &desc, MenuListCallback on_confirm,
				   MenuList *submenu)
			...
			try {
				for (int i = 0; i < values.size(); i++) {
					...
					if (v.type() == typeid(float)) {
						...
						if (std::any_cast<std::basic_string<char>>(initialVal) ==
							std::any_cast<std::basic_string<char>>(v)) {
...
MenuList::MenuList(MenuItemType type, const std::string &descp, std::vector<AbstractMenuItem *> items,
				   MenuListCallback on_change, MenuListCallback on_confirm)
	...
	if (cur && cur->isDeferred()) {
		...
		if (type != MenuItemType::Main && items.size() > scope.max_visible_options) {
			...
			if (scope.end < scope.count)
				...
				// GFX_blitAssetCPP(ASSET_SCROLL_DOWN, {}, surface, {rect.x, rect.h - SCALE1(PADDING + PILL_SIZE +
				// BUTTON_SIZE) + rect.y});
				GFX_blitAssetCPP(ASSET_SCROLL_DOWN, {}, surface,
								 {rect.x, rect.h - SCALE1(PADDING + PILL_SIZE) + rect.y});
		}
		...
		if (cur && cur->getDesc().length() > 0) {
			...
			GFX_blitTextCPP(font.tiny, description.c_str(), SCALE1(FONT_SMALL), uintToColour(THEME_COLOR4_255), surface,
							{(dst.x + dst.w - w) / 2, dst.y + dst.h - h, w, h});
		}
	}
}
...
void MenuList::drawFixedItem(SDL_Surface *surface, const SDL_Rect &dst, const AbstractMenuItem &item, bool selected)
{
	...
	if (item.getValue().has_value()) {
		...
		if (item.getType() == ListItemType::Color) {
			...
			SDL_Rect rect = {dst.x + dst.w - SCALE1(OPTION_PADDING + FONT_TINY),
							 dst.y + SCALE1(BUTTON_SIZE - FONT_TINY) / 2, SCALE1(FONT_TINY), SCALE1(FONT_TINY)};
...
#define COLOR_PADDING 4
			SDL_BlitSurfaceCPP(text, {}, surface,
							   {dst.x + mw - text->w - SCALE1(OPTION_PADDING + COLOR_PADDING + FONT_TINY),
								dst.y + ((dst.h - text->h) / 2)});
			...
			SDL_BlitSurfaceCPP(text, {}, surface,
							   {dst.x + mw - text->w - SCALE1(OPTION_PADDING), dst.y + ((dst.h - text->h) / 2)});
		SDL_FreeSurface(text);
	}

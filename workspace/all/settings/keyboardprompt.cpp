	...
	: MenuList(MenuItemType::Custom, title, {}, nullptr, on_confirm)
{
	state = {.redraw = true,
			 ...
			 .keyboard = {.display = true, .row = 0, .col = 0, .layout = 0, .title = title}};
}
...
void KeyboardPrompt::handleKeyboardInput(AppState &state)
{
	...
	if (PAD_justRepeated(BTN_UP)) {
		...
		if (state.keyboard.row < max_row - 1) {
			...
			if (state.keyboard.row + 1 == max_row - 1 &&
				(state.keyboard.col > center || (row_length & 1 && state.keyboard.col < center))) {
				offset = adjustOffsetEnterLastRow(offset, state.keyboard.col, center);
			}
			state.keyboard.col += offset;
			state.keyboard.row++;
...
void KeyboardPrompt::drawKeyboard(SDL_Surface *screen, const AppState &state)
{
	...
	if (!state.keyboard.title.empty()) {
		...
		SDL_Rect title_pos = {(screen->w - title->w) / 2, // center horizontally
							  20,						  // 20px from top
							  title->w, title->h};
	...
	SDL_Rect input_pos = {(screen->w) / 2, input_placeholder->h * 2, 0, input_placeholder->h};
	if (input != NULL) {
	...
	SDL_Rect input_bg = {40, input_placeholder->h * 2, screen->w - 80, input_placeholder->h};
	...
	for (int row = 0; row < num_rows; row++) {
		...
		for (int col = 0; col < len; col++) {
			...
			SDL_Rect key_pos = {start_x + (col * (current_key_width + column_spacing)),
								start_y + (row * (default_key_size + row_spacing)), current_key_width,
								default_key_size};
			...
			// center text in key
			SDL_Rect text_pos = {key_pos.x + (current_key_width - key_text->w) / 2,
								 key_pos.y + (default_key_size - key_text->h) / 2, key_text->w, key_text->h};

...
const char *triggernames[] = {"B",	 "A",	 "Y",	   "X",		"L",   "R",	 "FN1",
							  "FN2", "MENU", "SELECT", "START", "ALL", "LR", "DPAD"};

const char *effect_names[] = {"Linear",	 "Breathe",	 "Interval Breathe", "Static",	"Blink 1",
							  "Blink 2", "Blink 3",	 "Rainbow",			 "Twinkle", "Fire",
							  "Glitter", "NeonGlow", "Firefly",			 "Aurora",	"Reactive"};
const char *topbar_effect_names[] = {"Linear",		   "Breathe",	  "Interval Breathe", "Static",	 "Blink 1",
									 "Blink 2",		   "Blink 3",	  "Rainbow",		  "Twinkle", "Fire",
									 "Glitter",		   "NeonGlow",	  "Firefly",		  "Aurora",	 "Reactive",
									 "Topbar Rainbow", "Topbar night"};
const char *lr_effect_names[] = {"Linear",	"Breathe", "Interval Breathe", "Static",	 "Blink 1",	   "Blink 2",
								 "Blink 3", "Rainbow", "Twinkle",		   "Fire",		 "Glitter",	   "NeonGlow",
								 "Firefly", "Aurora",  "Reactive",		   "LR Rainbow", "LR Reactive"};
...
void save_settings()
{
	...
	if (file == NULL) {
		...
		for (int i = 0; i < maxlights; ++i) {
			...
			fprintf(file, "\n"); // added last extra break seperate because everytime i add another option i forget to
								 // change it lol
		}
...
void handle_light_input(LightSettings *light, SDL_Event *event, int selected_setting)
{
	const uint32_t bright_colors[] = {
		// Blues
		0x000011, 0x000022, 0x000033, 0x000044, 0x000055, 0x000066, 0x000077, 0x000088, 0x000099, 0x0000AA, 0x0000BB,
		0x0000CC, 0x3366FF, 0x4D7AFF, 0x6699FF, 0x80B3FF, 0x99CCFF, 0xB3D9FF, 0x0000FF,
		// Cyan
		0x001111, 0x002222, 0x003333, 0x004444, 0x005555, 0x006666, 0x007777, 0x008888, 0x009999, 0x00AAAA, 0x00BBBB,
		0x00CCCC, 0x33FFFF, 0x4DFFFF, 0x66FFFF, 0x80FFFF, 0x99FFFF, 0xB3FFFF, 0x00FFFF,
		// Green
		0x001100, 0x002200, 0x003300, 0x004400, 0x005500, 0x006600, 0x007700, 0x008800, 0x009900, 0x00AA00, 0x00BB00,
		0x00CC00, 0x33FF33, 0x4DFF4D, 0x66FF66, 0x80FF80, 0x99FF99, 0xB3FFB3, 0x00FF00,
		// Magenta
		0x110011, 0x220022, 0x330033, 0x440044, 0x550055, 0x660066, 0x770077, 0x880088, 0x990099, 0xAA00AA, 0xBB00BB,
		0xCC00CC, 0xFF33FF, 0xFF4DFF, 0xFF66FF, 0xFF80FF, 0xFF99FF, 0xFFB3FF, 0xFF00FF,
		// Purple
		0x220044, 0x330066, 0x440088, 0x5500AA, 0x6600CC, 0x7700DD, 0x8800EE, 0x9900FF, 0xAA00FF, 0xBB00FF, 0xCC00FF,
		0x8833FF, 0x994DFF, 0xAA66FF, 0xBB80FF, 0xCC99FF, 0xDDB3FF,
		// Red
		0x220000, 0x440000, 0x660000, 0x880000, 0xAA0000, 0xCC0000, 0xFF3333, 0xFF4D4D, 0xFF6666, 0xFF8080, 0xFF9999,
		0xFFB3B3, 0xFF0000,
		// Yellow
		0x222200, 0x444400, 0x666600, 0x888800, 0xAAAA00, 0xCCCC00, 0xFFFF33, 0xFFFF4D, 0xFFFF66, 0xFFFF80, 0xFFFF99,
		0xFFFFB3, 0xFFFF00,
		// Orange
		0x331100, 0x662200, 0x993300, 0xCC4400, 0xFF5500, 0xFF6600, 0xFF7711, 0xFF8822, 0xFF9933, 0xFFAA44, 0xFFBB55,
		0xFFCC66, 0xFFDD77, 0xFFEE88,
		// White to Black Gradient
		0x000000, 0x111111, 0x222222, 0x333333, 0x444444, 0x555555, 0x666666, 0x777777, 0x888888, 0x999999, 0xAAAAAA,
		0xBBBBBB, 0xCCCCCC, 0xDDDDDD, 0xFFFFFF};
...
int main(int argc, char *argv[])
{
	...
	while (!quit) {
		...
		if (dirty) {
			...
			char title[256];
			int text_width =
				GFX_truncateText(font.medium, light_name_text, title, max_width, SCALE1(BUTTON_PADDING * 2));
			...
			if (is_brick) {
				...
				const char *non_brick_labels[] = {"Effect", "Color", "Speed", "Brightness (All Leds)",
												  "Info brightness (All Leds)"};
				memcpy(settings_labels, non_brick_labels, sizeof(non_brick_labels)); // Copy values
			}
			int settings_values[5] = {lightsDefault[selected_light].effect, lightsDefault[selected_light].color1,
									  lightsDefault[selected_light].speed, lightsDefault[selected_light].brightness,
			...
			for (int j = 0; j < 5; ++j) {
				...
				if (j == 0) { // Display effect name instead of number
					snprintf(setting_text, sizeof(setting_text), "%s: %s", settings_labels[j],
							 selected_light == 3   ? lr_effect_names[settings_values[j] - 1]
							 : selected_light == 2 ? topbar_effect_names[settings_values[j] - 1]
					...
					SDL_BlitSurface(text, &(SDL_Rect){0, 0, text->w, text->h}, screen,
					...
					int text_width = text->w + SCALE1(BUTTON_PADDING * 2);
					GFX_blitPill(selected ? ASSET_WHITE_PILL : ASSET_BLACK_PILL, screen,
								 &(SDL_Rect){SCALE1(PADDING), y, text_width + SCALE1(BUTTON_MARGIN + BUTTON_SIZE),
											 SCALE1(PILL_SIZE)});
					SDL_BlitSurface(text, &(SDL_Rect){0, 0, text->w, text->h}, screen,
									&(SDL_Rect){SCALE1(PADDING + BUTTON_PADDING), y + SCALE1(4)});
					SDL_FreeSurface(text);

					GFX_blitAssetColor(ASSET_BUTTON, NULL, screen,
									   &(SDL_Rect){SCALE1(PADDING) + text_width, y + SCALE1(BUTTON_MARGIN)},
									   settings_values[j]);
				} else {

...
static void readyResumePath(char *rom_path, int type)
{
	...
	if (can_resume) {
		...
		int s = atoi(slot);
		sprintf(preview_path, "%s/.minui/%s/%s.%0d.bmp", SHARED_USERDATA_PATH, emu_name, rom_file,
				s); // /.userdata/.minui/<EMU>/<romname>.ext.<n>.bmp
...
static void openRom(char *path, char *last)
{
	...
	if (should_resume) {
		...
		if (has_m3u) {
			...
			sprintf(disc_path_path, "%s/.minui/%s/%s.%s.txt", SHARED_USERDATA_PATH, emu_name, rom_file,
...
void onThumbLoaded(SDL_Surface *surface)
{
	...
	GFX_ApplyRoundedCorners_RGBA8888(thumbbmp, &(SDL_Rect){0, 0, thumbbmp->w, thumbbmp->h},
...
void animcallback(finishedTask *task)
{
	...
	if (pillRect.w > 0 && pillRect.h > 0) {
		...
		if (task->done) {
			...
			SDL_Surface *cropped =
				SDL_CreateRGBSurfaceWithFormat(0, crop_rect.w, crop_rect.h, 32, SDL_PIXELFORMAT_RGBA8888);
			if (!cropped) {
...
int animWorker(void *unused)
{
	while (true) {
		...
		// This somehow leads to the pill not rendering correctly when wrapping the list (last element to first, or
		// reverse).
		// TODO: Figure out why this is here. Ideally we shouldnt refer to specific platforms in here, but the commit
		// message doesnt help all that much and comparing magic numbers also isnt that descriptive on its own.
...
int main(int argc, char *argv[])
{
	...
	SDL_LockMutex(animMutex);
	globalpill = SDL_CreateRGBSurfaceWithFormat(SDL_SWSURFACE, screen->w, SCALE1(PILL_SIZE), FIXED_DEPTH,
												SDL_PIXELFORMAT_RGBA8888);
	globalText = SDL_CreateRGBSurfaceWithFormat(SDL_SWSURFACE, screen->w, SCALE1(PILL_SIZE), FIXED_DEPTH,
												SDL_PIXELFORMAT_RGBA8888);
	...
	while (!quit) {
		...
		if (dirty) {
			...
			// NOTE:22 This causes slowdown when CFG_getMenuTransitions is set to false because animationdirection turns
			// > 0 somewhere but is never set back to 0 and so this code runs on every action, will fix later
			if (animationdirection != ANIM_NONE ||
				(lastScreen == SCREEN_GAMELIST && currentScreen == SCREEN_GAMESWITCHER)) {
				if (tmpOldScreen)
			...
			if (currentScreen == SCREEN_QUICKMENU) {
				...
				sprintf(newBgPath, SDCARD_PATH "/.media/quick_%s%s.png", current->name,
						!strcmp(current->name, "Wifi") && CFG_getWifi() || // wifi or wifi_off, based on state
								!strcmp(current->name, "Bluetooth") && CFG_getBluetooth()
							? "_off"
				...
				if (show_setting && !GetHDMI())
					...
					GFX_blitButtonGroup((char *[]){BTN_SLEEP == BTN_POWER ? "POWER" : "MENU", "SLEEP", NULL}, 0, screen,
										0);
...
#define MENU_LINE_MARGIN 8	 // space between top and bottom line
					...
					// available space
					int item_inset_x =
						(item_space_x - SCALE1(qm_slots * MENU_ITEM_SIZE + (qm_slots - 1) * MENU_ITEM_MARGIN)) / 2;
					...
					for (int c = 0; c < quick->count; c++) {
						...
						SDL_Rect text_rect = {item_rect.x + (item_size - w) / 2,
					...
					ox += (screen->w - SCALE1(PADDING + MENU_MARGIN_X + MENU_MARGIN_X + PADDING) -
				...
				GFX_clear(screen);
				GFX_flipHidden();
				GFX_animateSurfaceOpacity(tmpOldScreen, 0, 0, screen->w, screen->h, 255, 0,
				...
				if (recents->count > 0) {
					...
					// title pill
					{
						...
						char display_name[256];
						int text_width = GFX_truncateText(font.large, selectedEntry->name, display_name, max_width,
					...
					if (can_resume)
						...
						GFX_blitButtonGroup((char *[]){BTN_SLEEP == BTN_POWER ? "POWER" : "MENU", "SLEEP", NULL}, 0,
											screen, 0);
					...
					if (has_preview) {
						...
						if (bmp) {
							...
							if (lastScreen == SCREEN_GAME) {
								...
								GFX_flipHidden();
								GFX_animateSurfaceOpacity(bmp, 0, 0, screen->w, screen->h, 0, 255,
								...
								GFX_animateSurface(tmpNewScreen, 0, 0 - screen->h, 0, 0, screen->w, screen->h,
						...
						SDL_Surface *tmpsur =
							SDL_CreateRGBSurfaceWithFormat(0, screen->w, screen->h, 32, SDL_PIXELFORMAT_RGBA8888);
						SDL_FillRect(tmpsur, &preview_rect, SDL_MapRGBA(screen->format, 0, 0, 0, 255));
						if (lastScreen == SCREEN_GAME) {
							GFX_animateSurfaceOpacity(tmpsur, 0, 0, screen->w, screen->h, 255, 0,
													  CFG_getMenuTransitions() ? 150 : 20, LAYER_BACKGROUND);
						} else if (lastScreen == SCREEN_GAMELIST) {
							GFX_animateSurface(tmpsur, 0, 0 - screen->h, 0, 0, screen->w, screen->h,
											   CFG_getMenuTransitions() ? 100 : 20, 255, 255, LAYER_ALL);
						} else if (lastScreen == SCREEN_GAMESWITCHER) {
							GFX_flipHidden();
							if (gsanimdir == SLIDE_LEFT)
								GFX_animateSurface(tmpsur, 0 + screen->w, 0, 0, 0, screen->w, screen->h,
												   CFG_getMenuTransitions() ? 80 : 20, 0, 255, LAYER_ALL);
							else if (gsanimdir == SLIDE_RIGHT)
								GFX_animateSurface(tmpsur, 0 - screen->w, 0, 0, 0, screen->w, screen->h,
												   CFG_getMenuTransitions() ? 80 : 20, 0, 255, LAYER_ALL);
				...
				if (((entry->type == ENTRY_DIR || entry->type == ENTRY_ROM) && CFG_getRomsUseFolderBackground())) {
					...
					if ((strcmp(newBg, folderBgPath) != 0 || lastType != entry->type) && sizeof(folderBgPath) != 1) {
						...
						if (!exists(tmppath)) {
							// Safeguard: If no background is available, still render the text to leave the user a way
							// out
				...
				if (show_setting && !GetHDMI())
					...
					GFX_blitButtonGroup((char *[]){BTN_SLEEP == BTN_POWER ? "POWER" : "MENU",
				...
				if (total > 0) {
					...
					for (int i = top->start, j = 0; i < top->end; i++, j++) {
						...
						int available_width =
							MAX(0, (had_thumb ? ox + SCALE1(BUTTON_MARGIN) : screen->w - SCALE1(BUTTON_MARGIN)) -
									   SCALE1(PADDING * 2));
						...
						SDL_Color text_color = uintToColour(THEME_COLOR4_255);
						int notext = 0;
						if (selected_row == remember_row && j == selected_row &&
							(selected_row + 1 >= (top->end - top->start) || selected_row == 0 ||
							 selected_row == remember_row)) {
							text_color = uintToColour(THEME_COLOR5_255);
							notext = 1;
						}
						...
						if (j == selected_row) {
							is_scrolling =
								GFX_resetScrollText(font.large, display_name, max_width - SCALE1(BUTTON_PADDING * 2));
							...
							globalpill = SDL_CreateRGBSurfaceWithFormat(SDL_SWSURFACE, max_width, SCALE1(PILL_SIZE),
																		FIXED_DEPTH, SDL_PIXELFORMAT_RGBA8888);
						...
						SDL_Rect dest_rect = {SCALE1(BUTTON_MARGIN + BUTTON_PADDING),
											  SCALE1(PADDING + (j * PILL_SIZE)) + text_offset_y};
					...
					if (lastScreen == SCREEN_GAMESWITCHER) {
						if (switcherSur) {
							...
							GFX_flipHidden();
							GFX_animateSurface(switcherSur, 0, 0, 0, 0 - screen->h, screen->w, screen->h,
											   CFG_getMenuTransitions() ? 100 : 20, 255, 255, LAYER_BACKGROUND);
							animationdirection = ANIM_NONE;
						}
					}
			...
			if (animationdirection != ANIM_NONE) {
				if (CFG_getMenuTransitions()) {
					...
					if (animationdirection == SLIDE_LEFT)
						GFX_animateAndFadeSurface(tmpOldScreen, 0, 0, 0 - FIXED_WIDTH, 0, FIXED_WIDTH, FIXED_HEIGHT,
												  200, tmpNewScreen, 1, 0, FIXED_WIDTH, FIXED_HEIGHT, 0, 255,
												  LAYER_THUMBNAIL);
			...
			if (lastScreen == SCREEN_QUICKMENU) {
				...
				SDL_LockMutex(animMutex);
				if (list_show_entry_names) {
					GFX_drawOnLayer(globalpill, pillRect.x, pillRect.y, globallpillW, globalpill->h, 1.0f, 0,
									LAYER_TRANSITION);
					// GFX_drawOnLayer(globalText, SCALE1(PADDING+BUTTON_PADDING), pilltargetTextY, globalText->w,
					// globalText->h, 1.0f, 0, LAYER_SCROLLTEXT);
				}
				SDL_UnlockMutex(animMutex);
			}
			...
			// honestly this whole thing is here only for the scrolling text, I set it now to run this at 30fps which is
			// enough for scrolling text, should move this to seperate animation function eventually
			...
			if (currentScreen != SCREEN_GAMESWITCHER && currentScreen != SCREEN_QUICKMENU) {
				if (is_scrolling && pillanimdone && currentAnimQueueSize < 1) {
					...
					int available_width = (had_thumb ? ox + SCALE1(BUTTON_MARGIN) : screen->w - SCALE1(BUTTON_MARGIN)) -
										  SCALE1(PADDING * 2);
					...
					int text_width = GFX_getTextWidth(font.large, entry_text, cached_display_name, available_width,
													  SCALE1(BUTTON_PADDING * 2));
					int max_width = MIN(available_width, text_width);
					...
					GFX_clearLayers(LAYER_SCROLLTEXT);
					if (list_show_entry_names) {
						GFX_scrollTextTexture(font.large, entry_text, SCALE1(BUTTON_MARGIN + BUTTON_PADDING),
											  SCALE1(PADDING + remember_row * PILL_SIZE) + text_offset_y,
					...
					if (list_show_entry_names) {
						...
						GFX_drawOnLayer(globalText, SCALE1(BUTTON_MARGIN + BUTTON_PADDING), pilltargetTextY,
										globalText->w, globalText->h, 1.0f, 0, LAYER_SCROLLTEXT);
					}
					SDL_UnlockMutex(animMutex);
					PLAT_GPU_Flip();
				}
			...
			if (needDraw) {
				...
				// This should either be 16(.66666667) or make proper use of SDL_Ticks to only wait for the next render
				// pass.
				SDL_Delay(17);
			}
			SDL_UnlockMutex(animqueueMutex);

...
static void blitButton(char *label, SDL_Surface *dst, int pressed, int x, int y, int w)
{
	...
	if (len <= 2) {
		...
		SDL_BlitSurface(
			text, NULL, dst,
			&(SDL_Rect){point.x + (SCALE1(BUTTON_SIZE) - text->w) / 2, point.y + (SCALE1(BUTTON_SIZE) - text->h) / 2});
	} else {
		text = TTF_RenderUTF8_Blended(font.tiny, label, COLOR_BUTTON_TEXT);
		...
		SDL_BlitSurface(
			text, NULL, dst,
			&(SDL_Rect){point.x + (w - text->w) / 2, point.y + (SCALE1(BUTTON_SIZE) - text->h) / 2, text->w, text->h});
	}
	...
	while (!quit) {
		...
		if (dirty) {
			...
			// L group
			{
				...
				blitButton("L1", screen, PAD_isPressed(BTN_L1), x + SCALE1(BUTTON_MARGIN), y + SCALE1(BUTTON_MARGIN),
						   0);
				if (has_L2)
			...
			// DPAD group
			{
				...
				SDL_FillRect(screen,
							 &(SDL_Rect){x, y + SCALE1(PILL_SIZE / 2), SCALE1(PILL_SIZE), SCALE1(PILL_SIZE * 2)},
							 THEME_COLOR3);
				...
				SDL_FillRect(screen,
							 &(SDL_Rect){x + SCALE1(PILL_SIZE / 2), y, SCALE1(PILL_SIZE * 2), SCALE1(PILL_SIZE)},
							 THEME_COLOR3);
			...
			// ABXY group
			{
				...
				// SDL_FillRect(screen, &(SDL_Rect){x,y+SCALE1(PILL_SIZE/2),SCALE1(PILL_SIZE),SCALE1(PILL_SIZE*2)},
				// THEME_COLOR3);
				GFX_blitPillColor(ASSET_WHITE_PILL, screen, &(SDL_Rect){x, y, 0}, THEME_COLOR3, RGB_WHITE);
				...
				// SDL_FillRect(screen, &(SDL_Rect){x+SCALE1(PILL_SIZE/2),y,SCALE1(PILL_SIZE*2),SCALE1(PILL_SIZE)},
				// THEME_COLOR3);
				GFX_blitPillColor(ASSET_WHITE_PILL, screen, &(SDL_Rect){x, y, 0}, THEME_COLOR3, RGB_WHITE);

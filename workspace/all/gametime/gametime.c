// heavily modified from the Onion original:
// https://github.com/OnionUI/Onion/blob/main/src/playActivity/playActivityUI.c
#include <stdio.h>
#include <unistd.h>
#include <stdbool.h>
#include <signal.h>
	...
	if (textSurface != NULL) {
		...
		if (right_align)
			SDL_BlitSurface(textSurface, NULL, screen,
							&(SDL_Rect){rect->w - textSurface->w, rect->y, rect->w, rect->h});
		else
...
void renderRoundedRectangle(SDL_Rect rect, Uint32 color, int radius)
{
	...
	_drawFilledCircle(screen, rect.x + rect.w - radius - 1, rect.y + rect.h - radius - 1, radius,
					  color); // Bottom-right
}
...
SDL_Surface *loadRomImage(char *image_path)
{
	...
	SDL_Surface *dst = SDL_CreateRGBSurface(0, SCALE1(IMG_MAX_WIDTH), SCALE1(IMG_MAX_HEIGHT), ft->BitsPerPixel,
											ft->Rmask, ft->Gmask, ft->Bmask, ft->Amask);
...
void renderList(int count, int start, int end, int selected)
{
	...
	for (int index = start, row = 0; index < end; index++, row++) {
		...
		renderRoundedRectangle((SDL_Rect){layout.list_display_start_x, layout.list_display_start_y + row * elemHeight,
										  layout.list_display_size_x, elemHeight},
							   isSelected ? RGB_WHITE : RGB_BLACK, SCALE1(24));

		SDL_Surface *romImage = romImages[index];
		if (romImage) {
			SDL_Rect rectRomImage = {layout.list_display_start_x + num_width + thumbMargin / 2 +
										 (SCALE1(IMG_MAX_WIDTH) - romImage->w) / 2,
									 layout.list_display_start_y + elemHeight * row + thumbMargin / 2,
									 SCALE1(IMG_MAX_WIDTH), SCALE1(IMG_MAX_HEIGHT)};
		...
		renderText(rom_name, font.medium, textColor,
				   &(SDL_Rect){layout.list_display_start_x + num_width + thumbMargin + SCALE1(IMG_MAX_WIDTH),
							   layout.list_display_start_y + thumbMargin / 2 + elemHeight * row,
							   layout.list_display_size_x, textHeight});
		...
		SDL_Rect detailsRect = {layout.list_display_start_x + num_width + thumbMargin + SCALE1(IMG_MAX_WIDTH),
...
void initLayout()
{
	...
	// y: default padding, below the title pill and some additional padding to leave some breathing room
	// (BUTTON_MARGIN).
	...
	layout.list_display_rect.x = layout.list_display_start_x, layout.list_display_rect.y = layout.list_display_start_y,
	layout.list_display_rect.w = layout.list_display_size_x, layout.list_display_rect.h = layout.list_display_size_y;

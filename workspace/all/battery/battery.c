	...
	if (textSurface != NULL) {
		...
		if (right_align)
			SDL_BlitSurface(textSurface, NULL, screen,
							&(SDL_Rect){rect->x - textSurface->w, rect->y, rect->w, rect->h});
		else
		...
		if (rc == SQLITE_OK) {
			...
			while ((sqlite3_step(stmt) == SQLITE_ROW) && (!b_quit)) {
				...
				if ((is_charging) && (!is_estimation_computed)) {
					...
					if (previous_index <
						(graph.layout.graph_max_size - duration_to_pixel(GRAPH_MIN_SESSION_FOR_ESTIMATION))) {
						float slope = (float)(graph.graphic[graph.layout.graph_max_size - 1].pixel_height -
											  graph.graphic[previous_index].pixel_height) /
									  (float)(graph.layout.graph_max_size - 1 - previous_index);

						if (slope < 0) {
							estimation_line_size =
								(int)-(graph.graphic[graph.layout.graph_max_size - 1].pixel_height) / slope;

							int estimated_playtime =
								(int)(estimation_line_size)*GRAPH_DISPLAY_DURATION / graph.layout.graph_display_size_x;
							...
							if (estimated_playtime < GRAPH_MAX_PLAUSIBLE_ESTIMATION) {
								...
								total_duration +=
									estimated_playtime + (int)(GRAPH_ESTIMATED_LINE_GAP)*GRAPH_DISPLAY_DURATION /
								...
								for (int x = (graph.layout.graph_max_size - room_to_make);
									 x < graph.layout.graph_max_size; x++) {
									int y = graph.graphic[previous_index].pixel_height +
											(int)(slope * (x - previous_index));
									if (y > 0) {
...
void renderPage()
{
	...
	const struct SDL_Point br = {graph.layout.graph_display_start_x + graph.layout.graph_display_size_x,
								 graph.layout.graph_display_start_y + graph.layout.graph_display_size_y};
	...
	if (estimation_line_size == 0)
		current_index = graph.layout.graph_max_size -
						(graph.layout.graph_display_size_x * (int)(segment_duration / GRAPH_SEGMENT_HIGH));
	else
		current_index = begining_session_index;

	current_index -=
		(int)(current_page * (graph.layout.graph_display_size_x * (int)(segment_duration / GRAPH_SEGMENT_HIGH)) /
			  GRAPH_PAGE_SCROLL_SMOOTHNESS);
	...
	renderText(text_line, font.medium, COLOR_WHITE,
			   &(SDL_Rect){graph.layout.label_session_x, graph.layout.label_session_y, graph.layout.label_size_x,
						   graph.layout.label_size_y});

	sprintf(text_line, "Current: %s", current_percentage);
	renderText(text_line, font.medium, COLOR_WHITE,
			   &(SDL_Rect){graph.layout.label_current_x, graph.layout.label_current_y, graph.layout.label_size_x,
						   graph.layout.label_size_y});

	sprintf(text_line, "Remaining: %s", session_left);
	renderTextAlignRight(text_line, font.medium, COLOR_WHITE,
						 &(SDL_Rect){graph.layout.label_left_x, graph.layout.label_left_y, graph.layout.label_size_x,
									 graph.layout.label_size_y});

	sprintf(text_line, "Longest: %s", session_best);
	renderTextAlignRight(text_line, font.medium, COLOR_WHITE,
						 &(SDL_Rect){graph.layout.label_best_x, graph.layout.label_best_y, graph.layout.label_size_x,
									 graph.layout.label_size_y});
...
#define STATS_HEIGHT 31
	...
	// y: stretch whole height below graph_display_start_y, leaving room at the bottom for padding, button hints, stats,
	// axis labels
	graph.layout.graph_display_size_x = hw - SCALE1(PADDING * 2 + BUTTON_MARGIN * 2 + AXIS_MARGIN + AXIS_WIDTH);
	graph.layout.graph_display_size_y = hh - SCALE1(PADDING * 2 + PILL_SIZE * 2 + BUTTON_MARGIN * 2 + GRAPH_MARGIN * 2 +
													STATS_MARGIN * 2 + STATS_HEIGHT);
	...
	graph.layout.label_session_y =
		graph.layout.label_y + SCALE1(GRAPH_MARGIN + STATS_MARGIN + BUTTON_MARGIN + FONT_MEDIUM);
	...
	while (!quit) {
		...
		if (PAD_justPressed(BTN_MENU)) {
			...
			if (PAD_justRepeated(BTN_LEFT)) {
				dirty = 1;
				int page_max = (int)(((GRAPH_MAX_FULL_PAGES) * (GRAPH_PAGE_SCROLL_SMOOTHNESS)) /
									 (segment_duration / GRAPH_SEGMENT_HIGH));
		...
		if (dirty) {
			...
			// title pill
			{
				...
				char title[256];
				int text_width =
					GFX_truncateText(font.large, display_name, title, max_width, SCALE1(BUTTON_PADDING * 2));

...
///////////////////////////////
static int qualityLevels[] = {3, 4, 2, 1};
	...
	if (offset > 0) {
		...
		if (offset < -max_lost_frames * frame_duration) {
			...
			LOG_debug("%s: lost sync by more than %d frames (early ?!) @%llu -> reset\n\n", __FUNCTION__,
					  max_lost_frames, SDL_GetPerformanceCounter());
		} else if (offset < 0) {
...
scaler_t GFX_getAAScaler(GFX_Renderer *renderer)
{
	...
	// LOG_info("blend_denominator: %f (%i && %i)\n", blend_denominator, HAS_SKINNY_SCREEN,
	// renderer->dst_w>renderer->src_w);
...
void GFX_blitSurfaceColor(SDL_Surface *src, SDL_Rect *src_rect, SDL_Surface *dst, SDL_Rect *dst_rect,
						  uint32_t asset_color)
{
	...
	if (asset_color != RGB_WHITE) {
		...
		SDL_SetSurfaceColorMod(src, (asset_color >> 16) & 0xFF, (asset_color >> 8) & 0xFF, asset_color & 0xFF);
...
void GFX_blitButton(char *hint, char *button, SDL_Surface *dst, SDL_Rect *dst_rect)
{
	...
	if (strlen(button) == 1) {
		...
		GFX_blitPillDark(ASSET_BUTTON, dst,
...
void GFX_blitBatteryAtPosition(SDL_Surface *dst, int x, int y)
{
	...
	if (pwr.is_charging) {
		...
		GFX_blitAssetColor(percent <= 10 ? ASSET_BATTERY_LOW : ASSET_BATTERY, NULL, dst, &(SDL_Rect){x, y},
						   THEME_COLOR6);
		...
		if (CFG_getShowBatteryPercent()) {
			...
			SDL_Rect target = {x + (battery_rect.w - text->w) / 2 + 1, y + (battery_rect.h - text->h) / 2 - 1};
...
int GFX_blitHardwareGroup(SDL_Surface *dst, int show_setting)
{
	...
	if (show_setting && !GetHDMI()) {
		...
		if (!show_bt && !show_wifi && !show_clock) {
			...
			GFX_blitPillColor(ASSET_WHITE_PILL, dst, &(SDL_Rect){ox, oy, ow, SCALE1(PILL_SIZE)}, THEME_COLOR2,
							  RGB_WHITE);
			...
			if (show_wifi) {
				int asset = strength == SIGNAL_STRENGTH_HIGH  ? ASSET_WIFI
							: strength == SIGNAL_STRENGTH_MED ? ASSET_WIFI_MED
							: strength == SIGNAL_STRENGTH_LOW
								? ASSET_WIFI_LOW
...
float calculateBufferAdjustment(float remaining_space, float targetbuffer_over, float targetbuffer_under, int batchsize)
{
	...
	// I make crazy small adjustments, mooore tiny is mooore stable :D But don't come neir the limits cuz imma hit ya
	// with that 0.005 ratio adjustment, pow pow! I wonder if staying in the middle of 0 to 4000 with 512 samples per
	// batch playing at tiny different speeds each iteration is like the smallest I can get lets say hovering around
	// 2000 means 2000 samples queue, about 4 frames, so at 17ms(60fps) thats  68ms delay right? Should have payed
	// attention when my math teacher was talking dammit Also I chose 3 for pow, but idk if that really the best nr,
	// anyone good in maths looking at my code?
...
size_t SND_batchSamples(const SND_Frame *frames, size_t frame_count)
{
	...
	} else if (currentbufferfree >
			   snd.frame_count * 0.99f) { // if for some reason buffer drops below threshold again, pause it (like psx
										  // core can stop sending audio in between scenes or after fast forward etc)
		SND_pauseAudio(true);
	}
...
size_t SND_batchSamples_fixed_rate(const SND_Frame *frames, size_t frame_count)
{
	...
	currentbufferfree = remaining_space;
	// let audio buffer fill up a little before playing audio, so no underruns occur. Target fill rate of buffer is
	// about 50% so start playing when about 40% full
	if (currentbufferfree < snd.frame_count * 0.6f) {
		SND_pauseAudio(false);
	} else if (currentbufferfree >
			   snd.frame_count * 0.99f) { // if for some reason buffer drops below 1% again, pause audio again (like psx
										  // core can stop sending audio in between scenes or after fast forward etc)
		SND_pauseAudio(true);
	}
...
{ // plat_sound_init
	...
	snd.frame_count =
		((float)spec_out.freq / SCREEN_FPS) * 8; // buffer size based on sample rate out (times 12 samples headroom)
...
#define MENU_DELAY 250 // also in PWR_update()
	...
	if (PAD_justPressed(btn)) {
		ignore_menu = 0;
		menu_start = now;
	} else if (PAD_isPressed(btn) && BTN_MOD_BRIGHTNESS == btn &&
...
#define MOD_DELAY 250
	if (((PAD_isPressed(BTN_MOD_VOLUME) || PAD_isPressed(BTN_MOD_BRIGHTNESS) || PAD_isPressed(BTN_MOD_COLORTEMP)) &&
		 (!delay_settings || now - mod_unpressed_at >= MOD_DELAY)) ||
		((!BTN_MOD_VOLUME || !BTN_MOD_BRIGHTNESS || !BTN_MOD_COLORTEMP) &&

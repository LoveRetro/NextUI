...
// https://github.com/OnionUI/Onion/blob/main/src/playActivity/playActivityDB.h
#include <stdlib.h>
#include <stdbool.h>
#include <sys/stat.h>
...
sqlite3 *play_activity_db_open(void)
{
	...
	if (!db_exists) {
		...
		sqlite3_exec(game_log_db,
					 ...
					 "CREATE TABLE play_activity(rom_id INTEGER, play_time INTEGER, created_at INTEGER DEFAULT "
					 "(strftime('%s', 'now')), updated_at INTEGER);"
...
int play_activity_get_total_play_time(void)
{
	...
	char *sql = "SELECT SUM(play_time_total) FROM (SELECT SUM(play_time) AS play_time_total FROM play_activity GROUP "
				"BY rom_id) "
...
int __db_insert_rom(sqlite3 *game_log_db, const char *rom_type, const char *rom_name, const char *file_path,
					const char *image_path)
{
	...
	char *sql = sqlite3_mprintf("INSERT INTO rom(type, name, file_path, image_path) VALUES(%Q, %Q, %Q, %Q);", rom_type,
								rom_name, rel_path, image_path);
...
void __db_update_rom(sqlite3 *game_log_db, int rom_id, const char *rom_type, const char *rom_name,
					 const char *file_path, const char *image_path)
{
	char rel_path[MAX_PATH];
	__ensure_rel_path(rel_path, file_path);
...
int __db_get_orphan_rom_id(sqlite3 *game_log_db, const char *rom_path)
{
	...
	char *sql = sqlite3_mprintf("SELECT id FROM rom WHERE (name=%Q OR name=%Q) AND type='ORPHAN' LIMIT 1;", rom_name,
								file_name);
...
void play_activity_stop(char *rom_file_path)
{
	...
	char *sql = sqlite3_mprintf("UPDATE play_activity SET play_time = (strftime('%%s', 'now')) - created_at, "
								"updated_at = (strftime('%%s', 'now')) WHERE rom_id = %d AND play_time IS NULL;",
								rom_id);
	play_activity_db_execute(sql);
	sqlite3_free(sql);
}
...
void play_activity_stop_all(void)
{
	...
	play_activity_db_execute("UPDATE play_activity SET play_time = (strftime('%s', 'now')) - created_at, updated_at = "
							 "(strftime('%s', 'now')) WHERE play_time IS NULL;"

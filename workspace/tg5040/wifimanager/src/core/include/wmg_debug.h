...
enum { MSG_ERROR = 0, MSG_WARNING, MSG_INFO, MSG_DEBUG, MSG_MSGDUMP, MSG_EXCESSIVE };

#ifdef CONFIG_NO_STDOUT_DEBUG

#define wmg_printf(args...)                                                                                            \
	do {                                                                                                               \
	} while (0)
#define wmg_debug_open_file(p)                                                                                         \
	do {                                                                                                               \
	} while (0)
#define wmg_debug_close_file()                                                                                         \
	do {                                                                                                               \
	} while (0)
...
#define wmg_printf(level, fmt, arg...) wmg_print(level, "<%s:%u>:" fmt "", __FUNCTION__, __LINE__, ##arg)
#else
#define wmg_printf(level, fmt, arg...) wmg_print(level, fmt, ##arg)
#endif /*CONFIG_DEBUG_FUNCTION_LINE*/

void wmg_print(int level, const char *fmt, ...) PRINTF_FORMAT(2, 3);

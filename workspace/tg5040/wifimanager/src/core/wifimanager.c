...
static int aw_wifi_ssid_is_connected_ap(char *ssid)
{
	...
	if ((sta = get_wpa_status_info()) != NULL) {
		...
		if (sta->wpa_state == WPA_COMPLETED) {
			if (!strncmp(sta->ssid, ssid, strlen(ssid)) && sta->ip_address[0] != '\0')
				return 1;
		}
	} else {
...
{
	if (w->StaEvt.state == CONNECTING || w->StaEvt.state == OBTAINING_IP) {
		return -1;
	} else {
		return 0;
	}
...
static int aw_wifi_connect_ap(const char *ssid, const char *passwd, int event_label)
{
	...
	if (ret >= 0 || w->StaEvt.event == WSE_PASSWORD_INCORRECT || w->StaEvt.event == WSE_WPA_TERMINATING)
		goto end;
...
static const aw_wifi_interface_t aw_wifi_interface = {
	aw_wifi_add_state_callback,	 aw_wifi_ssid_is_connected_ap, aw_wifi_is_ap_connected,
	aw_wifi_connection_info,	 aw_wifi_get_scan_results,	   aw_wifi_connect_ap,
	aw_wifi_connect_ap_key_mgmt, aw_wifi_connect_ap_auto,	   aw_wifi_connect_ap_with_netid,
	aw_wifi_add_network,		 aw_wifi_disconnect_ap,		   aw_wifi_remove_network,
	aw_wifi_remove_all_networks, aw_wifi_list_networks,		   aw_wifi_get_netid,
	aw_wifi_get_status,			 aw_wifi_clear_network,		   aw_wifi_wps_pbc};

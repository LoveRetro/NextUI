...
static void kill_processes(int sig)
{
    ...
    closedir(proc);
    printf("poweroff_next: [DEBUG] kill_processes: Sent signal %d to %d processes\n", sig, killed_count);
}
...
{
    printf("poweroff_next: [DEBUG] swapoff_device: Attempting to swapoff %s\n", path);
    ...
    {
        printf("poweroff_next: [DEBUG] swapoff_device: Successfully turned off swap on %s\n", path);
    }
}
...
static void swapoff_all(void)
{
    ...
    fclose(swaps);
    printf("poweroff_next: [DEBUG] swapoff_all: Processed %d swap devices\n", swap_count);
}

static void safe_umount(const char *path, int flags)
{
    printf("poweroff_next: [DEBUG] safe_umount: Attempting to unmount %s with flags 0x%x\n", path, flags);
    if (umount2(path, flags) != 0)
    {
        if (errno == EINVAL || errno == ENOENT)
        {
            printf("poweroff_next: [DEBUG] safe_umount: %s not mounted or invalid\n", path);
            return;
        }
    ...
    {
        printf("poweroff_next: [DEBUG] safe_umount: Successfully unmounted %s\n", path);
    }
}
...
static void finalize_poweroff(void)
{
    ...
    {
        perror("poweroff_next: syscall(SYS_reboot, POWER_OFF) failed");
    }
    ...
    reboot(RB_POWER_OFF);
    perror("poweroff_next: All poweroff methods failed");
}
...
static void kill_sdcard_users(void)
{
    ...
    while ((entry = readdir(proc)) != NULL)
    {
        ...
        while ((fd_entry = readdir(fd_dir)) != NULL)
        {
            ...
            {
                printf("poweroff_next: [DEBUG] kill_sdcard_users: Killing PID %d (has fd to %s)\n", pid, target);
                kill(pid, SIGKILL);
                break;
            }
        }
...
static bool is_sdcard_mounted(void)
{
    ...
    while ((ent = getmntent(fp)) != NULL)
    {
        ...
        {
            printf("poweroff_next: [DEBUG] is_sdcard_mounted: Found %s mounted\n", SDCARD_PREFIX);
            mounted = true;
            break;
        }
    }
    ...
    endmntent(fp);
    printf("poweroff_next: [DEBUG] is_sdcard_mounted: Result = %s\n", mounted ? "true" : "false");
    return mounted;
}
...
static bool unmount_sdcard_with_retries(void)
{
    ...
    {
        printf("poweroff_next: [DEBUG] unmount_sdcard_with_retries: Attempt %d/3\n", attempt + 1);
        ...
        {
            printf("poweroff_next: [DEBUG] unmount_sdcard_with_retries: Success on attempt %d\n", attempt + 1);
            return true;
        }
    ...
    printf("poweroff_next: [DEBUG] unmount_sdcard_with_retries: Final result = %s\n", result ? "success" : "failed");
    return result;
}
...
static int axp2202_write_reg(int fd, uint8_t reg, uint8_t value)
{
    ...
    if (result != 0)
        printf("poweroff_next: [DEBUG] axp2202_write_reg: Failed to write 0x%02x to reg 0x%02x\n", value, reg);
    return result;
}

static int execute_axp2202_poweroff(void)
{
    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: Starting PMIC shutdown sequence\n");
    int fd = open(I2C_DEVICE, O_RDWR | O_CLOEXEC);
    if (fd < 0)
    {
        perror("poweroff_next: open(I2C_DEVICE)");
        return -1;
    }
    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: Opened I2C device\n");

    if (ioctl(fd, I2C_SLAVE, AXP2202_ADDR) < 0)
    {
        perror("poweroff_next: ioctl(I2C_SLAVE)");
        close(fd);
        return -1;
    }
    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: Set I2C slave address\n");

    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: Writing 0x00 to registers 0x40-0x44\n");
    for (int reg = 0x40; reg <= 0x44; ++reg)
        axp2202_write_reg(fd, (uint8_t)reg, 0x00);

    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: Writing 0xFF to registers 0x48-0x4C\n");
    for (int reg = 0x48; reg <= 0x4C; ++reg)
        axp2202_write_reg(fd, (uint8_t)reg, 0xFF);

    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: Writing 0x0A to reg 0x22\n");
    axp2202_write_reg(fd, 0x22, 0x0A);
    struct timespec wait = {.tv_sec = 0, .tv_nsec = 50000000};
    nanosleep(&wait, NULL);

    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: Writing final poweroff command (0x01 to reg 0x27)\n");
    int ret = axp2202_write_reg(fd, 0x27, 0x01);
    close(fd);
    printf("poweroff_next: [DEBUG] execute_axp2202_poweroff: PMIC poweroff command sent, result=%d\n", ret);
...
{
    puts("poweroff_next: Starting power-off protection sequence...");
    ...
    puts("poweroff_next: PMIC software power-off triggered.");
...
int main(void)
{
    ...
    printf("poweroff_next: [DEBUG] main: Power-off protection = %s\n", protection_enabled ? "enabled" : "disabled");

...
typedef enum { BT_PAIR_REQUEST_REJECTED, BT_PAIR_REQUEST_CANCELED } btmg_pair_request_error_t;
typedef struct {
	char *remote_address;
	char *address_type;
	char *remote_name;
...
#define BTMG_LE_ADV_CHANNEL_ALL                                                                                        \
	(BTMG_LE_ADV_CHANNEL_NONE | BTMG_LE_ADV_CHANNEL_37 | BTMG_LE_ADV_CHANNEL_38 | BTMG_LE_ADV_CHANNEL_39)
#ifdef DBUS_LE_ADV
typedef enum {
	ADV_BROADCAST,
	ADV_PERIPHERAL,
...
int bt_manager_gap_send_pair_error(void *handle, btmg_pair_request_error_t e, const char *err_msg);
...
int bt_manager_le_scan_start(int dev_id, btmg_le_scan_type_t scan_type, btmg_le_scan_param_t *scan_param,
							 void **scan_handle);
int bt_manager_le_scan_stop(int dev_id, void *scan_handle);
int bt_manager_le_disconnect(void);
int bt_manager_le_set_scan_parameters(int dev_id, btmg_le_scan_type_t scan_type, btmg_le_scan_param_t *scan_param,
									  uint8_t own_type, uint8_t filter_policy);
...
typedef void (*bt_avrcp_play_state_cb)(
	const char *bd_addr, btmg_avrcp_play_state_t state); /*used to report play state of avrcp, recommended*/
typedef void (*bt_avrcp_track_changed_cb)(const char *bd_addr,
										  btmg_track_info_t track_info); /*used to report track information*/
typedef void (*bt_avrcp_play_position_cb)(const char *bd_addr, int song_len,
...
typedef struct btmg_a2dp_source_callback_t {
	...
	bt_a2dp_source_audio_state_cb
		a2dp_source_audio_state_cb; /*used to report the a2dp_source audio state, not recommended as mentioned before*/
...
int bt_manager_gatt_client_connect(int dev_id, uint8_t *addr, btmg_bdaddr_type_t dst_type, uint16_t mtu,
								   btmg_security_level_t sec);
...
int bt_manager_gatt_client_write_request(void *cn_handle, int char_handle, uint8_t *value, uint16_t len);
int bt_manager_gatt_client_write_command(void *cn_handle, int char_handle, bool signed_write, uint8_t *value,
										 uint16_t len);
...
int bt_manager_gatt_client_discover_all_primary_services(void *cn_handle, uint16_t start_handle, uint16_t end_handle);
int bt_manager_gatt_client_discover_primary_services_by_uuid(void *cn_handle, const char *uuid, uint16_t start_handle,
															 uint16_t end_handle);

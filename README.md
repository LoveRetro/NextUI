...
### RetroArch compatible SRM Saves

By default, NextUI uses the same ROM save format as MinUI: `filename.ext.sav`

If you need your saves to be compatible with RetroArch (e.g., `filename.srm`), create an empty file named `use_srm_saves` and place it in the following directory: `.userdata/shared/`

**Example:** `.userdata/shared/use_srm_saves`

This will enable SRM save format for RetroArch compatibility.

**⚠ Warning**: This does not rename or convert existing save files. You will need to manually rename or migrate your old saves if necessary.
